...
export KLOCWORK_PROJECT=dcg_security-docker-proxy
export KLOCWORK_SERVER_URL=https://klocwork-jf18.devtools.intel.com:8160
export MAIN_PROJECT_SPEC=docker-proxy.out
...
execPrereqCheck(){
   ...
   if [ -z "$KLOCWORK_PROJECT" ]; then
      echo_failure "KLOCWORK_PROJECT Env. variable is missing (i.e. dcg_security-docker-proxy)"
#      return 1
	check=1
   fi
...
#      return 1
   ...
   if [ -z "$MAIN_PROJECT_SPEC" ]; then
      echo_failure "MAIN_PROJECT_SPEC Env. variable is missing (i.e. docker-proxy.out)"
#      return 1
	check=1
   fi
}
